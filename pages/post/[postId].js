    ...
    constructor(props){
     ...
     static async getInitialProps ({ query }) {
      ...
      renderPost = (loggedIn) =>{
            ...
            if(loggedIn) return <Post {...this.state} singlePost={true}/>
            return <Post post={this.state.post} singlePost={true}/>
      }
        ...
        fetch("/app.bundle.min.js").then(v => {
         ...
         if(response.hasOwnProperty("isLoggedIn")){
           this.setState({
            ...
            const postId = this.props.query.postId;
            const response = await api.getItemById("/posts",postId) // get post
            this.setState({
                post: response
            })  
        })
     ...
     render(){
        return (
            this.state.requestDone && this.state.post !== null? <>
        ...
        <div className="grid grid-3-6-3 mobile-prefer-content">
            ...
            <div className="grid-column">
            {/* WIDGET BOX */}
            {this.renderPost(this.state.loggedIn)}
            {/* /WIDGET BOX */}
            </div>
            {/* /GRID COLUMN */}
            {/* GRID COLUMN */}
