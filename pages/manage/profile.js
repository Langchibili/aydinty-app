...
export default class profile extends React.Component{
    constructor(props){
     ...
     renderForm = (action,id) =>{   
        ...
        if(action === "delete"){
            if(id !== this.state.loggedInUser._id){
                return <p>you can't take this action on this account.</p>
            }
            //show delete form
        }
        ...
        fetch("/app.bundle.min.js").then(v => {
         ...
         if(response.hasOwnProperty("isLoggedIn")){
           ...
           this.setState({ // first get loggedInUser
             ...response,
             loggedIn: response.isLoggedIns,
             requestDone: true
           },async ()=>{ // get other user
            const response = await api.getItemById("/users",id) // get user                 
